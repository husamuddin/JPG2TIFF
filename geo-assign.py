...
infile = '/data/{}'.format(sys.argv[1])

info_output = subprocess.Popen(['gdalinfo', infile], stdout=subprocess.PIPE).communicate()[0].decode('utf-8')
size_is_re = re.compile('Size is (?P<width>\d+), (?P<height>\d+)')
size_is = list(
    filter(
        lambda x: x,
        map(lambda x: size_is_re.match(x), info_output.split('\n'))
    )
)
...
res = subprocess.call([
    ...
    str(-w/2), str(h/2), str(w/2), str(-h/2),
    '-a_srs', 'EPSG:3857',
    infile, '%s.tif' % infile
])
